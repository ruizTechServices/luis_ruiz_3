...
export default async function SoundboardPage({ searchParams }: {
  ...
  const requested = (await searchParams).sound;
  const initialSoundId = typeof requested === "string" && SOUND_CLIPS.some((sound) => sound.id === requested) ? requested : null;
  ...
  return <main id="main-content" className="site-container py-12 sm:py-20">
    <header className="mb-10 grid items-end gap-6 md:grid-cols-[1.2fr_1fr] md:gap-16">
      <div>
        <p className="eyebrow mb-5">Gio’s soundboard / {SOUND_CLIPS.length} sounds</p>
    ...
    <Soundboard key={initialSoundId ?? "default"} initialSoundId={initialSoundId} />
